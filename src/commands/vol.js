...
export async function execute(interaction, db, config) {
  ...
  try {
    ...
    const target = await db.getUser(targetUser.id);
    
    if (targetUser.bot || targetUser.id === uid) {
      return interaction.reply({ content: '❌ Vous ne pouvez pas voler un bot ou vous-même.', flags: 64 });
    }
    
    // Vérifier le cooldown (1 vol par heure)
    const lastTheft = await db.getSetting(`last_theft_${uid}`) || 0;
    ...
    await db.adjustBalance(uid, -theftCost);
    await db.setSetting(`last_theft_${uid}`, now);
    ...
    if (success) {
      ...
      await db.adjustBalance(targetUser.id, -stolenAmount);
      await db.adjustBalance(uid, stolenAmount);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '💳 Votre solde', value: `${formatCents((await db.getUser(uid)).balance)} Ǥ`, inline: true },
    ...
    console.error('Erreur commande vol:', error);
